...
def get_ycrcb_mask(img, debug=False):
    ...
    lower_thresh = numpy.array([90, 100, 130], dtype=numpy.uint8)#90
    upper_thresh = numpy.array([230, 120, 180], dtype=numpy.uint8)#230
...
def process(img, thresh=0.5, debug=False):
    ...
    mask = numpy.where((mask_hsv + mask_rgb + mask_ycrcb)/3>255/2,255,0)
    #cv2.imshow('4',mask)
    #cv2.waitKey(0)
